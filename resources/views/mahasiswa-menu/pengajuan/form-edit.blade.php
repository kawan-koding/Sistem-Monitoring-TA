    ...
    <div class="col-md-8 col-sm-12 col-lg-8">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                <form action="{{route('mahasiswa.pengajuan-ta.update', ['id' => $data->id])}}" method="post" enctype="multipart/form-data">
                    @csrf
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 mb-3">
                        <div class="form-group">
                            ...
                            <select name="pemb_1" id="pemb_1" class="form-control select2" required>
                                ...
                                <option value="{{$item->id}}" {{$bimbingUji->dosen_id == $item->id ? "selected='selected'" : ''}}>({{($item->kuota_pemb_1-$item->total_pemb_1)}}) {{$item->nidn}} - {{$item->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
