                        ...
                        <tbody>
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge {{ isset($item->status) ? ($item->status == 'acc' ? 'badge-soft-success' : ($item->status == 'draft' ? 'bg-dark-subtle text-body' : ($item->status == 'pengajuan ulang' ? 'bg-dark-subtle text-body' : 'badge-soft-danger' ))) : '-'}} small mb-1"> {{ ucfirst($item->status ?? '-')}} </span>
