        ...
        <table class="table" width="100%">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $category->nama }}</td>
                    <td>{{ $bimbingUji->penilaian->where('kategori_nilai_id', $category->id)->where('type', 'Seminar')->first()->nilai ?? '0' }}</td>
                    <td>{{ grade($bimbingUji->penilaian->where('kategori_nilai_id', $category->id)->where('type', 'Seminar')->first()->nilai ?? 0) }}</td>
                </tr>
                @endforeach
            </tbody>
            <tfoot class="bg-light">
                <tr>
                    ...
                    <td colspan="2">{{ number_format($bimbingUji->penilaian->sum('nilai') > 0 ? $bimbingUji->penilaian->where('type', 'Seminar')->sum('nilai') / $bimbingUji->penilaian->count() : 0, 2, '.', '.') }}</td>
                </tr>
                <tr>
                    <td colspan="2">Total Nilai Huruf</td>
                    <td colspan="2">{{ grade($bimbingUji->penilaian->sum('nilai') > 0 ? $bimbingUji->penilaian->where('type', 'Seminar')->sum('nilai') / $bimbingUji->penilaian->count() : 0) }}</td>
                </tr>
            </tfoot>
        </table>
    </div>
