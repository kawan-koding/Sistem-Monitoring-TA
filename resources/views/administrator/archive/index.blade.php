        ...
        <div class="col-md-12 col-sm-12 col-g-12">
            <div class="card">
                ...
                <ul class="nav nav-tabs nav-tabs-custom nav-justified mt-1 mb-1" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link @if (url()->full() == route('apps.archives')) active @endif"
                            href="{{ route('apps.archives') }}">
                            <span class="d-block d-sm-none"><i class="bx bx-timer"></i></span>
                            <span class="d-none d-sm-block">Arsip Keseluruhan</span>
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link @if (url()->full() == route('apps.archives', ['type' => 'bimbing_uji'])) active @endif"
                            href="{{ route('apps.archives', ['type' => 'bimbing_uji']) }}">
                            <span class="d-block d-sm-none"><i class="bx bx-list-check"></i></span>
                            <span class="d-none d-sm-block">Arsip Bimbing Uji</span>
                        </a>
                    </li>
                </ul>
                @endif
                <div class="card-body">
                    <div
                        ...
                        <form action="" class="d-flex gap-2 flex-column flex-md-row align-items-start">
                            <input type="hidden" name="type" value="{{ request('type', 'bimbing_uji') }}">
                            <select name="program_studi" id="program_studi" class="form-control"
                                ...
                                <option value="semua" {{ request('program_studi') == 'semua' ? 'selected' : '' }}>Semua
                                    Program Studi</option>
                                @foreach ($prodi as $p)
                                    <option value="{{ $p->id }}"
                                        {{ request('program_studi') == $p->id ? 'selected' : '' }}>{{ $p->nama }}
                                    </option>
                                @endforeach
                            </select>
                            <select name="periode" class="form-control" onchange="this.form.submit()">
                                <option selected disabled hidden>Filter Periode</option>
                                <option value="semua" {{ request('periode') == 'semua' ? 'selected' : '' }}>Semua Periode
                                </option>
                            ...
                            <select name="jenis" class="form-control" onchange="this.form.submit()">
                                <option selected disabled hidden>Filter  Bimbing Uji</option>
                                <option value="semua">Semua</option>
                                <option value="pembimbing" {{ request('jenis') == 'pembimbing' ? 'selected' : '' }}>
                                    Pembimbing
                                </option>
                                <option value="penguji" {{ request('jenis') == 'penguji' ? 'selected' : '' }}>
                                    Penguji
                                </option>
                            </select>
                            @endif
                            ...
                            <tbody>
                                @foreach ($data as $key => $item)
                                    <tr>
                                        ...
                                        <td>
                                            <p class="m-0 badge rounded-pill bg-primary-subtle text-primary small">
                                                {{ $item->mahasiswa->programStudi->display }}</p>
                                            <a href="#" class="m-0" data-bs-toggle="modal"
                                                data-bs-target="#mahasiswaModal{{ $key }}">
                                                <p class="fw-bold m-0">{{ $item->mahasiswa->nama_mhs }}</p>
                                            </a>
                                            <div class="modal fade" id="mahasiswaModal{{ $key }}" tabindex="-1"
                                        ...
                                        <td>
                                            <span
                                                class="badge badge-soft-primary small mb-1 fw-bold">{{ isset($item->tipe) ? ($item->tipe == 'I' ? 'Individu' : 'Kelompok') : '-' }}</span>
                                            <p class="m-0 small"><strong>{{ $item->judul ?? '-' }}</strong></p>
                                            <p class="m-0 text-muted font-size-12 small">
                                        ...
                                        <td>
                                            ...
                                            <ol>
                                                @for ($i = 0; $i < 2; $i++)
                                                    @if (isset($item->tugas_akhir->bimbing_uji) &&
                                                            $item->tugas_akhir->bimbing_uji()->where('jenis', 'pengganti')->where('urut', $i + 1)->count() > 0)
                                        ...
                                        <td>
                                            <p class="small">{{ $item->periode_ta->nama ?? '-' }}</p>
                                        </td>
                                        <td>

                                            @php
                                                $isAdmin = session('switchRoles') == 'Admin';
                                                $isDosen = session('switchRoles') == 'Dosen';
                                                $isBimbingUji = request('type') == 'bimbing_uji';

                                                $showDetail = false;

                                                if ($isAdmin) {
                                                    $showDetail = true;
                                                } elseif ($isDosen && $isBimbingUji) {
                                                    $showDetail = true;
                                                }
                                            @endphp

                                            @if ($showDetail)
                                            <a href="{{ route('apps.archives.show', $item->id) }}"
                                                class="btn btn-sm btn-outline-warning mb-1" title="Detail">
