...
@section('content')
    <div class="">
        ...
        @endif
        <form action="{{route('apps.profile.update', $profile->id)}}" id="myFormulir" method="post" enctype="multipart/form-data">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-12 col-md-4 col-lg-4">
                            <div style="width: 200px; height: 200px; overflow: hidden; border-radius: 50%;" class="d-block mx-auto">
                                <img src="{{asset('storage/images/users/'. Auth::user()->image)}}" alt="" class="w-100">
                            </div>
                            <div class="d-flex justify-content-center">
                                <label for="fileInput" class="text-center p-0">
                                    <input type="file" id="fileInput" class="d-none" name="fileImage" onchange="this.form.submit()">
                                    <div class="btn btn-primary btn-sm mt-2 small"><i class="bx bx-camera small"></i> Ubah Foto</div>
                                </label>
                            </div>
                        @endif
            <form action="{{route('apps.profile.update', $profile->id)}}" id="myFormulir" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-sm-12 col-md-8 col-lg-8">
                                <div class="row">
                                    <div class="col-sm-12 col-md-6 col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" name="name" id="name" class="form-control" value="{{$profile->userable->nama_mhs ?? $profile->userable->name ?? ''}}">
                                        </div>
                                    </div>
                                    <div class="col-sm-12 col-md-6 col-lg-6">
                                        <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="jenis_kelamin" id="" class="form-control">
                                        <option value="Laki-laki" {{isset($profile->userable->jenis_kelamin) ? ($profile->userable->jenis_kelamin == 'Laki-laki' || $profile->userable->jenis_kelamin == 'Laki-laki' ? 'selected' : '') : ''}}>Laki-laki</option>
                                        <option value="Perempuan" {{isset($profile->userable->jenis_kelamin) ? ($profile->userable->jenis_kelamin == 'Perempuan' || $profile->userable->jenis_kelamin == 'Perempuan' ? 'selected' : '') : ''}}>Perempuan</option>
                                    </select>
                                </div>
                                @if ($profile->hasRole(['Dosen', 'Admin', 'Kaprodi']))    
                                <div class="mb-3">
                                    ...
                                    <div class="border p-4 text-center" style="border-radius: 8px">
                                        <input type="text" name="foto_profile" id="foto_profile" class="form-control filepond">
                                        @if (!is_null($profile->userable->ttd))
                                        <a href="{{asset('storage/images/dosen/'. $profile->userable->ttd)}}" class="btn btn-primary btn-sm w-100" target="_blank"> Lihat Tanda Tangan</a>
                                        @endif
                                    </div>
                                </div>
                                @endif
