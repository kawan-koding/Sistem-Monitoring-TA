...
@section('content')
    ...
    @if (getInfoLogin()->hasRole('Mahasiswa'))
        <div class="row">
            <div class="col-md-4 col-sm-12">
                <div
                    class="card text-center border-{{ isset($dataMahasiswa['tugasAkhir'])
                        ? ($dataMahasiswa['tugasAkhir']->status == 'acc'
                            ? 'accept'
                            : (in_array($dataMahasiswa['tugasAkhir']->status, ['draft', 'pengajuan ulang'])
                                ? 'draft'
                                : 'reject'))
                        : 'draft' }}">
                    <div class="card-body">
                        <p class="mb-2 fs-5">Pengajuan Tugas Akhir</p>
                        <i class="bx bx-{{ isset($dataMahasiswa['tugasAkhir'])
                            ? ($dataMahasiswa['tugasAkhir']->status == 'acc'
                                ? 'check-circle'
                                : (in_array($dataMahasiswa['tugasAkhir']->status, ['draft', 'pengajuan ulang'])
                                    ? 'timer'
                                    : 'x-circle'))
                            : 'timer' }} text-{{ isset($dataMahasiswa['tugasAkhir'])
                            ? ($dataMahasiswa['tugasAkhir']->status == 'acc'
                                ? 'success'
                                : (in_array($dataMahasiswa['tugasAkhir']->status, ['draft', 'pengajuan ulang'])
                                    ? 'dark'
                        ...
                        <h5
                            class="mb-0 text-{{ isset($dataMahasiswa['tugasAkhir'])
                                ? ($dataMahasiswa['tugasAkhir']->status == 'acc'
                                    ? 'success'
                                    : (in_array($dataMahasiswa['tugasAkhir']->status, ['draft', 'pengajuan ulang'])
                                        ? 'dark'
                                        : ($dataMahasiswa['tugasAkhir']->status == 'reject'
                                            ? 'danger'
                                            : 'danger')))
                                : 'dark' }}">
                            {{ isset($dataMahasiswa['tugasAkhir'])
                                ? ($dataMahasiswa['tugasAkhir']->status == 'acc'
                                    ...
                                    : (in_array($dataMahasiswa['tugasAkhir']->status, ['draft', 'pengajuan ulang'])
                                        ...
                                        : ($dataMahasiswa['tugasAkhir']->status == 'reject'
                                            ? 'Ditolak'
                                            : ($dataMahasiswa['tugasAkhir']->status == 'revisi'
                                                ? 'Revisi'
                                                : ($dataMahasiswa['tugasAkhir']->status == 'cancel'
                                                    ? 'Tidak Dilanjutkan'
                                                    : 'Menunggu')))))
                                : 'Belum Diajukan' }}
                        </h5>
                    ...
                    <div class="card-body">
                                        ...
                                        <div class="d-flex align-items-start">
                                            <div class="flex-1 overflow-hidden">
                                                <h6 class="m-0"><b>{{ $item->judul }}</b></h4>
                                                    ...
                                                    <p class="text-muted small m-0"><span class="me-2"><i
                                                                class="bx bx-user me-1"></i> {{ $item->dosen->name }}</span>
                                                        <i
                                                            class="bx bx-group me-1"></i>{{ $item->ambilTawaran()->where('status', 'Disetujui')->count() }}/{{ $item->kuota }}
                                                        Kuota | Jumlah Pengambil :
                                                        {{ $item->ambilTawaran()->where('status', '!=', 'Ditolak')->count() }}
                                                    </p>
                                            </div>
                                        </div>
                                    </a>
                        ...
                        @if ($dataMahasiswa['topik']->count() > 2)
                            <div class="text-center">
                                <a href="{{ route('apps.rekomendasi-topik') }}" class="btn btn-primary btn-sm">Lihat
                                    Selengkapnya...</a>
                            </div>
                        @endif
                    </div>
    ...
    <div class="col-lg-3 col-md-6">
        ...
        <div class="row">
            <div class="col-lg-6">
                <div class="card mb-3" style="height: 100%; min-height: 400px;">
                    <div class="card-body">
                ...
                <div class="card shadow-sm mb-3">
                    <div class="card-header bg-white py-3">
                        <h5 class="d-flex align-items-center gap-2 mb-0">
                            <span
                                class="w-100 fw-bold fs-4">{{ \Carbon\Carbon::now()->locale('id')->translatedFormat('F Y') }}</span>
                            <button class="btn btn-outline-primary btn-sm rounded-pill py-1 px-2"><i
                    ...
                    <div class="card-body p-0 border-top">
                        <div class="d-flex align-items-center">
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px" class="text-danger">
                                Min</div>
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px">Sen</div>
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px">Sel</div>
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px">Rab</div>
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px">Kam</div>
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px">Jum</div>
                            <div style="width: calc(100% / 7);text-align: center;padding: 5px 10px" class="text-danger">
                                Sab</div>
                        </div>
                        <div class="d-flex align-items-center flex-wrap">
                            @if (\Carbon\Carbon::now()->startOfMonth()->format('D') == 'Sun')
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                            @endif

                            @if (\Carbon\Carbon::now()->startOfMonth()->format('D') == 'Sat')
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                            ...
                            @if (\Carbon\Carbon::now()->startOfMonth()->format('D') == 'Fri')
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                            ...
                            @if (\Carbon\Carbon::now()->startOfMonth()->format('D') == 'Thu')
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                            ...
                            @if (\Carbon\Carbon::now()->startOfMonth()->format('D') == 'Wed')
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                            ...
                            @if (\Carbon\Carbon::now()->startOfMonth()->format('D') == 'Tue')
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">
                                </div>
                            ...
                            @for ($i = 0; $i < \Carbon\Carbon::now()->daysInMonth; $i++)
                                @if(\Carbon\Carbon::now()->day($i + 1)->isWeekend())
                                    <div style="width: calc(100% / 7);text-align: center;padding: 20px" class="text-danger">{{ $i + 1 }}
                                    </div>
                                @else
                                <div style="width: calc(100% / 7);text-align: center;padding: 20px">{{ $i + 1 }}
                                </div>
                                @endif
                            @endfor
                            </>
